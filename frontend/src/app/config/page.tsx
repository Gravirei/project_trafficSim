    ...
    <ProtectedRoute requiredRole="ADMIN">
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          >
            ...
            >
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                ...
                <input
                  ...
                  placeholder="e.g. East Lane"
                  style={inputStyle}
                />
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <label
                  style={{ fontSize: '0.875rem', color: 'var(--accent-green)', fontWeight: 500 }}
                >
                  GREEN (s)
                </label>
                <input
                  type="number"
                  value={newSignal.green_duration}
                  onChange={(e) =>
                    setNewSignal((p) => ({
                      ...p,
                      green_duration: Math.max(1, parseInt(e.target.value) || 1),
                    }))
                  }
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <label
                  style={{ fontSize: '0.875rem', color: 'var(--accent-yellow)', fontWeight: 500 }}
                >
                  YELLOW (s)
                </label>
                <input
                  type="number"
                  value={newSignal.yellow_duration}
                  onChange={(e) =>
                    setNewSignal((p) => ({
                      ...p,
                      yellow_duration: Math.max(1, parseInt(e.target.value) || 1),
                    }))
                  }
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <label
                  style={{ fontSize: '0.875rem', color: 'var(--accent-red)', fontWeight: 500 }}
                >
                  RED (s)
                </label>
                <input
                  type="number"
                  value={newSignal.red_duration}
                  onChange={(e) =>
                    setNewSignal((p) => ({
                      ...p,
                      red_duration: Math.max(1, parseInt(e.target.value) || 1),
                    }))
                  }
            ...
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              <button
                ...
                disabled={creating || !newSignal.name.trim()}
              >
        ...
        {loading ? (
          <div
            className="glass-panel"
            style={{ textAlign: 'center', color: 'var(--text-secondary)', padding: '4rem' }}
          >
          ...
          <div
            ...
            style={{ textAlign: 'center', color: 'var(--text-secondary)', padding: '4rem' }}
          >
            No signals configured. Click &ldquo;New Signal&rdquo; to add one.
          </div>
        ) : (
          <div
            style={{
              ...
              return (
                ...
                >
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    {[
                      {
                        label: 'GREEN DURATION (s)',
                        field: 'green_duration' as keyof Signal,
                        color: 'var(--accent-green)',
                        border: 'rgba(16,185,129,0.3)',
                      },
                      {
                        label: 'YELLOW DURATION (s)',
                        field: 'yellow_duration' as keyof Signal,
                        color: 'var(--accent-yellow)',
                        border: 'rgba(245,158,11,0.3)',
                      },
                      {
                        label: 'RED DURATION (s)',
                        field: 'red_duration' as keyof Signal,
