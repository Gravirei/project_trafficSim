...
import { Signal, SignalStats } from '@/types';
...
export default function AnalyticsPage() {
  ...
  const fetchAnalytics = useCallback(async () => {
    setLoading(true);
    try {
      const [summaryData, signalsRes] = await Promise.all([
        api.getAnalyticsSummary(),
        api.getSignals(),
      ]);
      setSummary(summaryData as AnalyticsSummary);

      const stats = await Promise.all(signalsRes.data.map((s: Signal) => api.getSignalStats(s.id)));
  ...
  useEffect(() => {
    // See useSignals.ts for rationale on the mount-fetch pattern.
      ...
      >
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)' }}>
            High-level operational metrics and system efficiency aggregates across all traffic
            signals.
          </p>
        </div>
      ...
      >
        ...
        >
          ...
          >
            ...
            <span
              style={{ fontSize: '1.25rem', color: 'var(--text-secondary)', marginLeft: '0.5rem' }}
            >
              veh
            </span>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div className="glass-panel" style={{ padding: 0, overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
              ...
              <tbody>
                {signalStats.map((stat: SignalStats) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: '1rem', fontFamily: 'var(--font-fira-code)' }}>
                        {stat.queueLength} veh
                      </td>
                      <td style={{ padding: '1rem', fontFamily: 'var(--font-fira-code)' }}>
                        {Number(stat.avgWaitTime).toFixed(2)}s
                      </td>
                      <td
                        style={{
                          padding: '1rem',
                      ...
                      <td style={{ padding: '1rem', fontFamily: 'var(--font-fira-code)' }}>
                        {Number(stat.arrivalRate).toFixed(2)} /s
                      </td>
                    </tr>
                  );
                })}
