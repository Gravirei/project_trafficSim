...
function DashboardContent() {
  const { data, history, connected } = useSocket();
  const { status, start, stop, reset, setMode, setSpeed, setArrivalRate, setAdaptiveThreshold } = useSimulation();
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
        <div>
          ...
          <p style={{ color: "var(--text-secondary)" }}>
            Live status: <span style={{ color: connected ? "var(--accent-green)" : "var(--accent-red)" }}>
        ...
        {data && (
          <div className="glass-panel" style={{ padding: "0.5rem 1rem", display: "flex", alignItems: "center", gap: "1rem" }}>
            ...
            <span style={{
              color: data.mode === 'ADAPTIVE' ? "var(--cta)" : "var(--accent-secondary)",
              fontWeight: 600
            }}>
              {data.mode}
            </span>
            ...
            {data.adaptiveAction && (
              <>
                <div style={{ width: "1px", height: "20px", background: "var(--border)" }}></div>
                <span className="metrics-label" style={{ color: "var(--accent-green)" }}>{data.adaptiveAction}</span>
              </>
            )}
          </div>
        )}
      ...
      <ControlPanel
        ...
        onSetAdaptiveThreshold={setAdaptiveThreshold}
      />

      {/* Unstable system warning */}
      ...
      {/* Real-Time Queue Chart */}
      {history.length > 0 && <QueueChart history={history} />}
      ...
      {data?.signals && data.signals.length > 0 ? (
        ...
        <div className="glass-panel" style={{ textAlign: "center", padding: "4rem", color: "var(--text-secondary)" }}>
          {connected ? "Waiting for simulation data..." : "Please connect to the backend server to view live intersections."}
        </div>
      )}
    </div>
  );
