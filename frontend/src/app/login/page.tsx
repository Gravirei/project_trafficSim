...
function LoginForm() {
  ...
  const initialShowForm = searchParams.get('action') === 'login';
  ...
  // Keep showForm in sync with the URL param (handles client-side nav).
  useEffect(() => {
    if (searchParams.get('action') === 'login') {
      // setState in effect: the URL param is a side-effect of client-side
      // navigation, so syncing form visibility from it is a valid
      // external-system subscription pattern.
    ...
    >
      ...
      {!showForm ? (
        ...
        >
          ...
          >
            Advanced adaptive traffic management with real-time queue modeling and AI-driven signal
            optimization.
          </p>
