...
function useScrollReveal(threshold = 0.15) {
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(
      // setState in event handler: the observer fires on intersection
      // events (not synchronously in the effect body), which is the
      // documented valid pattern for external-system subscription.
      ([entry]) => {
...
function SelectRole({
  ...
  onSelect: (role: 'VIEWER' | 'ADMIN') => void;
  onClose: () => void;
}) {
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
      <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
        <div
          style={{ fontSize: '1.5rem', fontWeight: 700, color: '#F8FAFC', margin: '0 0 0.5rem' }}
        >
          Select Your Role
        </div>
        ...
        {[
          {
            role: 'ADMIN' as const,
            label: 'Admin',
            desc: 'Full access to control and monitor the system.',
          },
          {
            role: 'VIEWER' as const,
            label: 'Viewer',
            desc: 'Read-only access to view dashboards and analytics.',
          },
        ].map((r) => (
          <button
            key={r.role}
...
function ViewerWarning({ onBack, onClose }: { onBack: () => void; onClose: () => void }) {
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', paddingTop: '2rem' }}>
      ...
      >
        The viewer role is currently under development. Please sign in as an admin to access the
        traffic control system.
      </div>

      <div
        style={{ height: '1px', background: 'rgba(255,255,255,0.06)', marginBottom: '1.5rem' }}
      />
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '2rem' }}>
        ...
        <div>
          ...
          <div
            style={{ color: '#64748B', fontSize: '0.8rem', fontFamily: "'DM Sans', sans-serif" }}
          >
      ...
      >
        ...
        <button
          ...
          onMouseOver={(e) => {
            if (!loading) {
              e.currentTarget.style.background = '#1a9e4b';
            }
          }}
          onMouseOut={(e) => {
            if (!loading) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ fontWeight: 700, fontSize: '1.25rem', color: '#F8FAFC' }}>TrafficSim</span>
      </Link>
    ...
    >
      ...
      <div
        style={{
          ...
          inset: 0,
          background:
            'linear-gradient(135deg, rgba(2, 6, 35, 0.85) 0%, rgba(2, 6, 35, 0.6) 50%, rgba(2, 6, 35, 0.85) 100%)',
        }}
      />
      ...
      >
        ...
        >
          Advanced adaptive traffic management powered by real-time queue modeling, AI-driven signal
          optimization, and live WebSocket monitoring.
        </p>
      ...
      <FadeInSection>
        <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
          ...
          <p style={{ ...styles.subtext, margin: '0 auto' }}>
            From real-time monitoring to AI-powered optimization, our platform provides
            comprehensive tools for modern traffic management.
          </p>
        </div>
      </FadeInSection>
          ...
          <FadeInSection key={i} delay={i * 0.1}>
            <div
              ...
              onMouseOver={(e) => {
                ...
                e.currentTarget.style.boxShadow = '0 20px 60px rgba(34, 197, 94, 0.05)';
              }}
              onMouseOut={(e) => {
                e.currentTarget.style.transform = 'none';
    ...
    >
      <FadeInSection>
        <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
          ...
          <p style={{ ...styles.subtext, margin: '0 auto' }}>
            From configuration to real-time optimization — get your traffic simulation running in
            minutes.
          </p>
        </div>
      </FadeInSection>
...
function CtaSection({ onOpenLogin }: { onOpenLogin: () => void }) {
  return (
    <FadeInSection>
      ...
      >
        <h2 style={{ ...styles.heading, marginBottom: '1rem' }}>Ready to Optimize Your Traffic?</h2>
        <p style={{ ...styles.subtext, margin: '0 auto 2rem auto', maxWidth: '500px' }}>
          Launch the dashboard to start monitoring and controlling your traffic simulation in real
          time.
        </p>
        <button
          onClick={onOpenLogin}
...
function Footer() {
  return (
    ...
    >
      ...
      <p style={{ margin: 0 }}>
        &copy; {new Date().getFullYear()} Traffic Control System. All rights reserved.
      </p>
    </footer>
  );
}
