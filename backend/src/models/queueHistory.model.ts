...
export interface AggregatedSummary {
  system_avg_wait: number | string;
  peak_queue_length: number | string;
...
export const QueueHistoryModel = {
  ...
  async getAggregatedSummary(): Promise<AggregatedSummary> {
    const result = await pool.query(`
            SELECT
            ...
            FROM queue_history
        `);
    return result.rows[0] as AggregatedSummary;
  },
};
