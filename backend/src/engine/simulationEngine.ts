...
export class SimulationEngine {
  ...
  async initialize(): Promise<void> {
    ...
    logger.info(
      { signalCount: this.signals.size, groupA: this.groupA, groupB: this.groupB },
  ...
  private syncFSMsToPhase(): void {
    ...
    for (const id of redGroup) {
      ...
      if (unit) {
        const redTime =
          greenState === 'GREEN' ? this.phaseTimer + this.yellowDuration : this.phaseTimer; // We're in yellow, they stay red for just the yellow timer
  ...
  private getPhaseGroups(): [number[], number[], 'GREEN' | 'YELLOW', string] {
    switch (this.phase) {
      case 'A_GREEN':
        return [this.groupA, this.groupB, 'GREEN', 'A'];
      case 'A_YELLOW':
        return [this.groupA, this.groupB, 'YELLOW', 'A'];
      case 'B_GREEN':
        return [this.groupB, this.groupA, 'GREEN', 'B'];
      case 'B_YELLOW':
        return [this.groupB, this.groupA, 'YELLOW', 'B'];
    }
  }
  ...
  private async tick(): Promise<void> {
    ...
    if (this.mode === 'ADAPTIVE' && (this.phase === 'A_GREEN' || this.phase === 'B_GREEN')) {
      ...
      if (activeMaxQueue === 0 && this.phaseTimer > 0) {
        ...
        activeMaxQueue > this.adaptiveThreshold &&
        activeMaxQueue > inactiveMaxQueue &&
        this.adaptiveExtension < this.maxAdaptiveExtension
      ) {
        ...
        if (fsm.getState() === 'GREEN') {
          ...
          for (const vehicle of servedVehicles) {
            if (vehicle.dbId !== undefined) {
              VehicleLogModel.logServed(vehicle.dbId).catch((err) =>
                logger.error({ err }, 'VehicleLog served error')
              );
            }
          }
        }
    ...
    QueueHistoryModel.insertBatch(historyBatch).catch((err) =>
      logger.error({ err }, 'QueueHistory batch insert error')
    );
  ...
  private advancePhase(): void {
    ...
    const [, , activeState] = this.getPhaseGroups();
    const groupName = this.phase.startsWith('A') ? 'N/S' : 'E/W';
    logger.info(
      { phase: this.phase, groupName, activeState, timer: this.phaseTimer },
      'Phase transition'
    );
